  ...
  <div class="schools-management-page">
    ...
    <header class="page-top-header">
      <div class="title-group">
        <h1 class="management-title">Schools</h1>
    ...
    <BaseCard class="table-card-wrapper">
      <BaseTable
        ...
        :is-loading="schoolStore.isLoading"
        @view="handleView"
        @edit="handleEdit"
        @delete="handleDelete"
      >
      ...
      <!-- Pagination -->
      <div v-if="schoolStore.schools?.length > 0" class="pagination-footer">
        <BasePagination
          v-model:page="currentPage"
          :per-page="itemsPerPage"
          :total-items="schoolStore.schools?.length || 0"
        />
      </div>
    </BaseCard>

    <div class="stats-footer-grid">
      <StatCard label="Total Items" :value="schoolStore.schools?.length || 0" icon="bi-layers" iconColor="blue" :isLoading="schoolStore.isLoading" />
      <StatCard label="Active" :value="activeCount" icon="bi-check-circle" iconColor="green" :isLoading="schoolStore.isLoading" />
      <StatCard label="Recently Added" :value="newlyAddedCount" icon="bi-clock-history" iconColor="yellow" :isLoading="schoolStore.isLoading" />
    </div>

    <!-- Modals Section -->
    <BaseModal
      :show="showFormModal"
      :title="isEditing ? 'Update School' : 'Add New School'"
      :subtitle="isEditing ? 'Update your old entry for schools' : 'Create a new entry for schools'"
      :hasPatternHeader="true"
      @close="showFormModal = false"
    >
      <div class="form-group">
        <label>School Name <span class="required">*</span></label>
        <input v-model="form.name" type="text" class="custom-input" placeholder="Enter school name...">
      </div>
      <template #footer>
        <button class="modal-btn cancel" :disabled="schoolStore.isProcessing" @click="showFormModal = false">Cancel</button>
        <button class="modal-btn confirm" :disabled="schoolStore.isProcessing" @click="saveSchool">
    ...
    <BaseModal
      ...
      subtitle="View complete information about this school"
    ...
    >
      ...
      <div class="details-grid">
        ...
        <div class="detail-card full">
          <label><i class="bi bi-card-text"></i> School Name</label>
    ...
    >
      ...
      <div class="confirmation-content">
        <p class="confirm-message">
          ...
          This will permanently remove the school and all associated data from the system.
        </p>
      ...
      <template #footer>
        <button class="modal-btn cancel" :disabled="schoolStore.isProcessing" @click="showDeleteModal = false">Cancel</button>
        <button class="modal-btn delete-confirm" :disabled="schoolStore.isProcessing" @click="confirmDelete">
          <span v-if="schoolStore.isProcessing" class="spinner-border spinner-border-sm me-2"></span>
          Delete School
        </button>
      </template>
...
const newlyAddedCount = computed(() => {
  ...
  return (schoolStore.schools || []).filter(s => new Date(s.created_at || Date.now()) > sevenDaysAgo).length
})

const paginatedSchools = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value
  const end = start + itemsPerPage.value
  return (schoolStore.schools || []).slice(start, end)
})

// Reset pagination when data changes
watch(() => schoolStore.schools?.length, () => {
  currentPage.value = 1
})
...
<style scoped>
.schools-management-page { display: flex; flex-direction: column; gap: 24px; }
